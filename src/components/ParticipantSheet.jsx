...
export function ParticipantSheet({ participant, inInitiative, onClose, onEdit, onJoinInit, onLeaveInit, onTracker, onDeleteTracker, onAddStatus, onRemoveStatus, onNote }) {
  ...
  return <Sheet title={participant.name} onClose={onClose}><p>{participant.description}</p><div className="grid2"><button className="primary" onClick={onEdit}>Modifier</button>{inInitiative ? <button className="small-btn" onClick={onLeaveInit}>Quitter l’init</button> : <button className="small-btn" onClick={onJoinInit}>Rejoindre init</button>}</div><button className="small-btn" style={{ width: '100%', marginTop: 8 }} onClick={onClose}>Fermer</button><h3>Suivis</h3><div className="stack">{participant.trackers.map((tracker) => <div className="tracker-sheet-row" key={tracker.id}><Tracker tracker={tracker} beforeTitle={eye(tracker)} onChange={(next) => onTracker(tracker.id, next)} onDelete={() => onDeleteTracker(tracker.id)} /></div>)}</div><h3>États</h3><div className="statuses">{participant.statuses?.map((s) => <Status key={s.id} status={s} onRemove={() => onRemoveStatus(s.id)} />)}<button className="small-btn" onClick={onAddStatus}>+ état</button></div><label className="field">Note<textarea value={participant.note || ''} onChange={(e) => onNote(e.target.value)} /></label></Sheet>;
}
