...
export function EditSheet({ participant, onClose, onSave, onDelete, onSaveTemplate }) {
  ...
  const saveDraftAsTemplate = () => onSaveTemplate?.(normalizeDraft(draft));

  return <Sheet title="Modifier" onClose={onClose}><label className="field">Nom<input value={draft.name} onChange={(e) => patch('name', e.target.value)} /></label><label className="field">Description<textarea value={draft.description || ''} onChange={(e) => patch('description', e.target.value)} /></label><div className="grid2"><label className="field">Type<select value={draft.kind} onChange={(e) => patch('kind', e.target.value)}>{participantKinds.map((k) => <option key={k}>{k}</option>)}</select></label><NumberField label="Initiative" value={draft.initiative} onChange={(value) => patch('initiative', value)} /></div><label className="field">Symbole<div className="statuses">{symbols.map((s) => <button key={s} className="small-btn" onClick={() => patch('symbol', s)}>{s}</button>)}</div></label><label className="field">Couleur<div className="statuses">{colors.map((c) => <button key={c} className="small-btn" onClick={() => patch('color', c)}>{colorNames[c] || c}</button>)}</div></label><h3>Suivis</h3><div className="stack tracker-list">{draft.trackers.map((tracker) => <TrackerEditor key={tracker.id} tracker={tracker} onChange={(next) => updateTracker(tracker.id, next)} onDelete={() => setDraft((d) => ({ ...d, trackers: d.trackers.filter((t) => t.id !== tracker.id) }))} />)}<button className="primary add-tracker-btn" onClick={() => setDraft((d) => ({ ...d, trackers: [...d.trackers, newTracker('bar')] }))}>Ajouter un suivi</button></div><button className="small-btn" style={{ width: '100%', marginTop: 12 }} onClick={saveDraftAsTemplate}>Enregistrer comme template</button>{confirmDelete && <div className="delete-confirm"><strong>Supprimer cette fiche ?</strong><span className="muted">Cette action retire le personnage de la scène.</span><div className="grid2"><button className="small-btn" onClick={() => setConfirmDelete(false)}>Annuler</button><button className="danger-btn" onClick={onDelete}>Confirmer</button></div></div>}<div className="grid2" style={{ marginTop: 12 }}><button className="primary" onClick={() => onSave(normalizeDraft(draft))}>Valider</button><button className="danger-btn" onClick={() => setConfirmDelete(true)}>Supprimer la fiche</button></div></Sheet>;
}
