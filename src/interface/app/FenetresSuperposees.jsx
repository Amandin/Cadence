...
export function FenetresSuperposees({ scene, restorePoints, dark, characters, templates, actions, etatInterface, commandesInterface, compteurGlobal, resolutionHorloge, templatesUi }) {
  ...
  const changerInitiativeCreneau = (participantId, indexCreneau, initiative) => characters.updateCharacter(participantId, (participant) => {
    const slots = normaliserCreneauxAction(participant, { initiativeTextOrder: scene.initiativeTextOrder });
    const modifies = slots.map((slot, index) => index === indexCreneau ? { ...slot, initiative, order: index } : { ...slot, order: index });
    const actionSlots = normaliserCreneauxAction({ ...participant, initiative: modifies[0]?.initiative ?? initiative, actionSlots: modifies }, { initiativeTextOrder: scene.initiativeTextOrder });
  ...
  const changerInitiativesCreneaux = (participantId, initiatives) => characters.updateCharacter(participantId, (participant) => {
    ...
    const actionSlots = normaliserCreneauxAction({ ...participant, initiative: valeurs[0], actionSlots: valeurs.map((initiative, index) => ({ id: `slot-${index + 1}`, initiative, order: index })) }, { initiativeTextOrder: scene.initiativeTextOrder });
  ...
  return <>
    ...
    {initiativeEntryOpen && <FenetreLancerInitiatives initiativeTextOrder={scene.initiativeTextOrder} participants={scene.participants} reserve={scene.reserve} onFermer={fermerSaisieInitiatives} onValider={actions.applyInitiativeRolls} onPasserHorsInitiative={actions.moveParticipantsToReserve} />}
  </>;
}
