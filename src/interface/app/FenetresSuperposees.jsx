...
export function FenetresSuperposees({ scene: sourceScene, sceneStream, randomSystem, restorePoints, dark, themeState, onThemeModeChange, characters, templates, actions, etatInterface, commandesInterface, compteurGlobal, resolutionHorloge, templatesUi }) {
  ...
  const creneauxManuelsActifs = (participant) => isManualMultipleActionMode(scene) && rulesAllowMultipleSlots(scene, participant);
  const optionsInitiative = { initiativeTextOrder: scene.initiativeTextOrder, multipleActionSlots: (participant) => rulesAllowMultipleSlots(scene, participant) };
  ...
  const ouvrirOptionsDepuisMenu = () => { fermerMenu?.(); ouvrirOptions?.(); };
  const modifierEtatSceneDepuisMenu = (statusId) => { fermerMenu?.(); ouvrirEtatScene?.(statusId); };
  ...
  return <>
    {characters.selected && <FicheParticipant participant={characters.selected} enInitiative={characters.isInInit(characters.selected.id)} initiativeTextOrder={scene.initiativeTextOrder} phaseActionMode={scene.phaseActionMode} phaseCount={scene.phaseCount} multipleActionSlots={creneauxManuelsActifs} utiliserInitiative={utiliserInitiative} initiativeBonusEnabled={initiativeBonusEnabled} initiativeBonusRollDefinitionId={scene.initiativeBonusRollDefinitionId} tiebreakerVisible={scene.tiebreakerVisible !== false} randomSystem={randomSystem} onFermer={characters.closeCharacter} onModifier={() => characters.editCharacter(characters.selected.id)} onBasculerDissimule={() => characters.updateCharacter(characters.selected.id, (participant) => ({ ...participant, secret: !participant.secret }))} onChangerInitiative={(indexCreneau, initiative) => changerInitiativeCreneau(characters.selected.id, indexCreneau, initiative)} onChangerInitiatives={(initiatives, phaseActions) => changerInitiativesCreneaux(characters.selected.id, initiatives, phaseActions)} onRejoindreInitiative={() => characters.requestJoin(characters.selected.id)} onQuitterInitiative={() => characters.leaveInit(characters.selected.id)} onInfoRapide={(index, valeur) => changerInfoRapide(characters.selected.id, index, valeur)} onSuivi={(trackerId, next) => characters.updateCharacterTracker(characters.selected.id, trackerId, next)} onSupprimerSuivi={(trackerId) => characters.deleteCharacterTracker(characters.selected.id)} onAjouterEtat={() => characters.requestStatus(characters.selected.id)} onModifierEtat={(statusId) => characters.requestStatus(characters.selected.id, statusId)} onRetirerEtat={(statusId) => characters.removeCharacterStatus(characters.selected.id, statusId)} onNote={(note) => characters.updateCharacter(characters.selected.id, (participant) => ({ ...participant, note }))} />}
    {characters.editing && <FenetreEditionFiche participant={characters.editing} initiativeTextOrder={scene.initiativeTextOrder} phaseActionMode={scene.phaseActionMode} phaseCount={scene.phaseCount} multipleActionSlots={creneauxManuelsActifs} utiliserInitiative={utiliserInitiative} initiativeBonusEnabled={initiativeBonusEnabled} allowActivationAutomation={allowActivationAutomation} categoryOrder={scene.categoryOrder} participantTypes={scene.participantTypes} tiebreakerVisible={scene.tiebreakerVisible !== false} tiebreakerLabel={scene.tiebreakerLabel} trackerTemplates={templates.trackerTemplates} randomSystem={randomSystem} onClose={characters.closeEditor} onSave={characters.saveCharacter} onDelete={() => characters.deleteCharacter(characters.editing.id)} onSaveTemplate={ouvrirSauvegardeTemplate} onSaveTrackerTemplate={templates.saveTrackerAsTemplate} />}
    {characters.statusTarget && <FenetreEtat key={characters.statusEdited?.id || characters.statusTarget.id} participant={characters.statusTarget} initialStatus={characters.statusEdited} defaultAdvanceOn={allowActivationAutomation ? 'activation' : 'round'} afficherChoixEvolution={allowActivationAutomation} allowActivationAdvance={allowActivationAutomation} statusTemplates={statusTemplates} onSaveTemplate={templates.saveStatusAsTemplate} onFermer={characters.cancelStatus} onValider={characters.saveStatus} />}
    ...
    {globalSheetOpen && <FenetreCompteurGlobal compteur={scene.globalTracker} sceneCounterTemplates={templates.sceneCounterTemplates} onModifier={actions.updateGlobalTracker} onChanger={actions.stepGlobal} onSaveTemplate={templates.saveSceneCounterAsTemplate} onFermer={fermerCompteurGlobal} />}
    ...
    {openMenu && <MenuPrincipal scene={scene} sceneStream={sceneStream} restorePoints={restorePoints} onRestore={restaurerScene} onReturnToPreparation={retourPreparation} onReturnToPreparationWithOptions={retourPreparationAvecOptions} onAdvanceRound={avancerRound} onDecreaseRound={reculerRound} onChangeRoundWithAutomations={changerRoundAvecAutomatismes} onAdvanceAutomations={avancerAutomatismes} onRewindAutomations={reculerAutomatismes} onResetTrackers={resetSuivisScene} onClearStatuses={effacerEtatsScene} onEndTemporaryEffects={terminerEffetsTemporaires} onClose={fermerMenu} dark={dark} themeState={themeState} onThemeModeChange={onThemeModeChange} onOpenOptions={ouvrirOptionsDepuisMenu} onAddParticipant={commandesInterface.ouvrirAjoutPersonnage} onDuplicateParticipant={characters.duplicateCharacter} onOpenCampaignHub={ouvrirHub} onGlobalTracker={actions.updateGlobalTracker} onStepGlobalTracker={actions.stepGlobal} onOpenGlobalTracker={ouvrirCompteurScene} onAddSceneStatus={ouvrirEtatSceneDepuisMenu} onEditSceneStatus={modifierEtatSceneDepuisMenu} onRemoveSceneStatus={actions.removeSceneStatus} onUpdateSceneNotes={(notes) => actions.updateSceneField('notes', notes)} onOpenInitiativeRoller={utiliserInitiative ? commandesInterface.ouvrirSaisieInitiatives : null} />}
    {initiativeEntryOpen && <FenetreLancerInitiatives initiativeTextOrder={scene.initiativeTextOrder} initiativeEnabled={utiliserInitiative} initiativeBonusEnabled={initiativeBonusEnabled} initiativeBonusRollDefinitionId={scene.initiativeBonusRollDefinitionId} randomSystem={randomSystem} multipleActionSlots={creneauxManuelsActifs} categoryOrder={scene.categoryOrder} participants={participantsSaisieInitiative} reserve={reserveSaisieInitiative} prefillExisting={initiativeEntryInitialLaunch} surpriseSelectionEnabled={initiativeEntryInitialLaunch && !!scene.preparationSurprise} tiebreakerVisible={scene.tiebreakerVisible !== false} tiebreakerLabel={scene.tiebreakerLabel} onFermer={fermerSaisieInitiatives} onValider={validerSaisieInitiatives || actions.applyInitiativeRolls} onPasserHorsInitiative={actions.moveParticipantsToReserve} />}
  </>;
}
