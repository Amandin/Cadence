...
import { initiativeTextOrderFromRandomSource, initiativeTextOrderPresetIds, initiativeTextOrderPresets, normalizeInitiativeTextOrder, presetInitiativeTextOrder, sameInitiativeTextOrder } from '../../domain/initiativeTextOrder.js';
...
export function FenetreInitiativeTextuelleEdition({ config, cardSources = [], tableSources = [], initiativeTextPresets = [], onFermer, onValider, onEnregistrerPreset }) {
  const labelSources = useMemo(() => [...cardSources, ...tableSources], [cardSources, tableSources]);
  const initial = useMemo(() => {
    const normalized = ensureParts(config);
    const source = labelSources.find((item) => item.id === normalized.sourceId);
    return source ? ensureParts(initiativeTextOrderFromRandomSource(source, { enabled: normalized.enabled })) : normalized;
  }, [config, labelSources]);
  ...
  const isLinkedSource = !!normalizedDraft.sourceId;
  const sourcePresetAvailable = labelSources.length > 0;
  const presetOptionIds = [initiativeTextOrderPresetIds.POSTURES];
  const defaultLabelSource = () => labelSources.find((source) => source.id === normalizedDraft.sourceId)
    || labelSources.find((source) => source.id === 'standard-54-cards')
    || labelSources[0];
  const cardSummary = parts
    .map((part) => `${part.label}: ${part.values.slice(0, 8).join(', ')}${part.values.length > 8 ? ` +${part.values.length - 8}` : ''}`)
    .join(' · ');
  const linkedSource = labelSources.some((source) => source.id === normalizedDraft.sourceId);
  const canValidate = !isLinkedSource || linkedSource;
  ...
  const appliquerPreset = (value) => {
    if (value.startsWith('custom:')) {
      ...
      setDraft((current) => ({ ...ensureParts(customPreset.config), enabled: current.enabled, preset: '', sourceId: '', cardSourceId: '' }));
      return;
    }
    ...
    setDraft((current) => {
      ...
      if (value === 'source') {
        const source = defaultLabelSource();
        return source ? ensureParts(initiativeTextOrderFromRandomSource(source, { enabled: current.enabled })) : current;
      }
      return { ...ensureParts(presetInitiativeTextOrder(value)), enabled: current.enabled, sourceId: '', cardSourceId: '' };
    });
  };
  const changerSource = (sourceId) => setDraft((current) => {
    const source = labelSources.find((item) => item.id === sourceId);
    if (!source) return current;
    return ensureParts(initiativeTextOrderFromRandomSource(source, { enabled: current.enabled }));
  });
  ...
  const presetValue = isLinkedSource ? 'source' : selectedCustomPresetId
  ...
  return (
    <>
      <Fenetre title={t('initiativeText.title')} onClose={demanderFermeture} header={header}>
        <div className="stack">
          <label className="field">{t('initiativeText.preset')}<select value={presetValue} onChange={(event) => appliquerPreset(event.target.value)}><option value="">{t('initiativeText.custom')}</option><option value="source" disabled={!sourcePresetAvailable}>{t('initiativeText.linkedSource')}</option>{initiativeTextPresets.map((preset) => <option key={preset.id} value={`custom:${preset.id}`}>{preset.name}</option>)}{presetOptionIds.map((id) => <option key={id} value={id}>{initiativeTextOrderPresets[id].label}</option>)}</select></label>
          {isLinkedSource && <>
            <label className="field">{t('initiativeText.linkedSource')}<select value={normalizedDraft.sourceId || ''} onChange={(event) => changerSource(event.target.value)} disabled={!sourcePresetAvailable}><option value="" disabled>{t('initiativeText.noLinkedSource')}</option>{labelSources.map((source) => <option key={source.id} value={source.id}>{source.name}</option>)}</select></label>
            <p className="muted compact-help">{t('initiativeText.linkedSourceHelp')}</p>
            {!sourcePresetAvailable && <p className="rule-warning">{t('initiativeText.linkedSourceRequired')}</p>}
