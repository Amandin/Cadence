...
import { multipleActionModeFromRules, normalizeInitiativeCostQuickCosts, normalizeInitiativeCostThreshold } from '../../domain/initiativeCost.js';
...
function OptionsActionsMultiplesCampagne({ scene, availability, onModifier }) {
  ...
  return <div className="scene-options compact-options advanced-rule-block"><h3>Actions multiples</h3><p className="muted compact-help">Choix exclusif pour les actions multiples dans le round.</p><div className="advanced-radio-list"><label className={`advanced-radio ${mode === multipleActionModes.NONE ? 'selected' : ''}`}><input type="radio" name="campaign-multiple-actions" checked={mode === multipleActionModes.NONE} onChange={() => setMode(multipleActionModes.NONE)} /><span><strong>Aucune</strong><small>Une seule activation d'initiative par personnage.</small></span></label><label className={`advanced-radio ${mode === multipleActionModes.MANUAL ? 'selected' : ''} ${manualDisabled ? 'disabled' : ''}`}><input type="radio" name="campaign-multiple-actions" checked={mode === multipleActionModes.MANUAL} disabled={manualDisabled} onChange={() => setMode(multipleActionModes.MANUAL)} /><span><strong>Creneaux manuels</strong><small>Le MJ saisit plusieurs creneaux au lancement.</small>{manualDisabled && <small className="rule-option-warning">{availability.reason}</small>}</span></label><label className={`advanced-radio ${mode === multipleActionModes.INITIATIVE_COST ? 'selected' : ''} ${costDisabled ? 'disabled' : ''}`}><input type="radio" name="campaign-multiple-actions" checked={mode === multipleActionModes.INITIATIVE_COST} disabled={costDisabled} onChange={() => setMode(multipleActionModes.INITIATIVE_COST)} /><span><strong>Actions multiples avec cout d'initiative</strong><small>Apres une action, le cout cree un nouveau creneau plus bas si le seuil le permet.</small>{costDisabled && <small className="rule-option-warning">{costAvailability.reason}</small>}</span></label></div>{mode === multipleActionModes.INITIATIVE_COST && <div className="initiative-cost-rule-fields"><label className="field">Seuil de fin de round<input type="number" inputMode="numeric" value={scene?.initiativeCostThreshold ?? defaultInitiativeCostThreshold} onChange={(event) => onModifier({ initiativeCostThreshold: event.target.value })} /></label><p className="rule-option-note">Si l'initiative apres cout est inferieure ou egale a ce seuil, le personnage ne recoit pas de nouveau creneau ce round.</p><label className="field">Couts rapides<input value={quickCosts} onChange={(event) => onModifier({ initiativeCostQuickCosts: event.target.value.split(',').map((item) => item.trim()) })} /></label><label className={`reset-switch campaign-rule-switch ${limiteCoutActif ? 'active' : ''}`}><span>Interdire de depenser plus d'initiative que le creneau actuel</span><input type="checkbox" checked={!!limiteCoutActif} onChange={(event) => onModifier({ initiativeCostLimitToCurrent: event.target.checked })} /></label></div>}</div>;
}
