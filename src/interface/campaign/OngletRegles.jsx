...
import { initiativeTextOrderEnabled, initiativeTextOrderPresetIds, initiativeTextOrderPresetLabel, normalizeInitiativeTextOrder, presetInitiativeTextOrder } from '../../domain/initiativeTextOrder.js';
...
function OptionsTemplatesReglesCampagne({ scene, ruleTemplates = [], rulePresetSnapshot = null, dernierTemplateId, nomCourant, editionNom, sauvegardeOk, onChangerNom, onEditerNom, onFermerEditionNom, onAppliquer, onEnregistrer, onSupprimer }) {
  ...
  const [familleId, setFamilleId] = useState(() => {
    const templateFamilies = new Set(ruleTemplates.map((template) => template.family || rulePresetFamilies.PERSONAL));
    return templateFamilies.has(rulePresetSnapshot?.family)
      ? rulePresetSnapshot.family
      : ruleTemplates[0]?.family || rulePresetFamilies.PERSONAL;
  });
  ...
  useEffect(() => {
    ...
    if (snapshotFamily && familles.some((famille) => famille.id === snapshotFamily) && snapshotFamily !== familleId) {
      setFamilleId(snapshotFamily);
      return;
    }
    if (!familles.some((famille) => famille.id === familleId)) {
      setFamilleId(familles[0]?.id || rulePresetFamilies.PERSONAL);
    }
...
function OptionsUtiliserInitiativeCampagne({ scene, onModifier }) {
  ...
  if (!souple) return <OptionsRelanceInitiativeCampagne scene={scene} onModifier={onModifier} />;
  return <><div className="scene-options compact-options advanced-rule-block"><label className={`reset-switch campaign-rule-switch ${utiliseInitiative ? 'active' : ''}`}><span>{t('rules.initiativeOrder.use')}</span><input type="checkbox" checked={utiliseInitiative} onChange={(event) => onModifier({ flexibleUseInitiative: event.target.checked })} /></label><p className="muted compact-help">{t('rules.initiativeOrder.flexibleHelp')}</p>{!utiliseInitiative && <p className="rule-option-note">{t('rules.initiativeOrder.noInitiativeNote')}</p>}</div>{utiliseInitiative && <OptionsRelanceInitiativeCampagne scene={scene} onModifier={onModifier} />}</>;
}
...
function OptionsDepartageCampagne({ scene, rollDefinitions = [], onModifier }) {
  ...
  const tiebreakerSwitchLabel = actif ? 'rules.tiebreaker.showWithBonus' : 'rules.tiebreaker.showWithoutBonus';
  if (initiativeTextOrderEnabled(scene?.initiativeTextOrder)) {
    return <div className="scene-options compact-options advanced-rule-block"><h3>{t('rules.tiebreaker.title')}</h3><p className="muted compact-help">{t('rules.tiebreaker.help')}</p><label className={`reset-switch campaign-rule-switch ${visible ? 'active' : ''}`}><span>{t('rules.tiebreaker.showWithoutBonus')}</span><input type="checkbox" checked={visible} onChange={(event) => onModifier({ tiebreakerVisible: event.target.checked })} /></label>{visible ? <label className="field">{t('rules.tiebreaker.label')}<input value={label} onChange={(event) => onModifier({ tiebreakerLabel: event.target.value })} /></label> : <p className="rule-option-note">{t('rules.tiebreaker.hiddenNote')}</p>}</div>;
  }
  return <div className="scene-options compact-options advanced-rule-block"><h3>{t('rules.initiativeBonus.title')}</h3><p className="muted compact-help">{t('rules.initiativeBonus.help')}</p><label className={`reset-switch campaign-rule-switch ${actif ? 'active' : ''}`}><span>{t('rules.initiativeBonus.enable')}</span><input type="checkbox" checked={!!actif} onChange={(event) => onModifier({ initiativeBonusEnabled: event.target.checked })} /></label><label className="field">{t('rules.initiativeBonus.roll')}<select value={selectedRollId} onChange={(event) => onModifier({ initiativeBonusRollDefinitionId: event.target.value })}><option value="">{t('rules.initiativeBonus.noRoll')}</option>{rollDefinitions.map((definition) => <option key={definition.id} value={definition.id}>{definition.name}</option>)}</select></label>{selectedRollId ? <p className="rule-option-note">{t('rules.initiativeBonus.rollNote')}</p> : !actif ? <p className="rule-option-note">{t('rules.initiativeBonus.noBonusNote')}</p> : null}<div className="advanced-rule-divider" /><h4>{t('rules.tiebreaker.title')}</h4><p className="muted compact-help">{t('rules.tiebreaker.help')}</p><label className={`reset-switch campaign-rule-switch ${visible ? 'active' : ''}`}><span>{t(tiebreakerSwitchLabel)}</span><input type="checkbox" checked={visible} onChange={(event) => onModifier({ tiebreakerVisible: event.target.checked })} /></label>{visible ? <label className="field">{t('rules.tiebreaker.label')}<input value={label} onChange={(event) => onModifier({ tiebreakerLabel: event.target.value })} /></label> : <p className="rule-option-note">{t('rules.tiebreaker.hiddenNote')}</p>}</div>;
}
...
function OptionsInitiativeTextuelleCampagne({ scene, availability, cardSources = [], tableSources = [], onModifier, onConfigurer }) {
  const config = normalizeInitiativeTextOrder(scene?.initiativeTextOrder);
  const linkedSource = [...cardSources, ...tableSources].find((source) => source.id === (config.sourceId || config.cardSourceId));
  const unlinkedSourcePreset = config.preset === 'source' && !linkedSource;
  const configLabels = config.parts.length && !unlinkedSourcePreset
    ? { ...config, enabled: true }
...
export function OngletRegles({ scene, campaignProfile, rulePresetSnapshot, onModifierRegles, ruleTemplates, initiativeTextPresets = [], cardSources = [], tableSources = [], rollDefinitions = [], embedded = false, onOuvrirProfilCampagne, onAppliquerTemplateRegles, onEnregistrerTemplateRegles, onEnregistrerPresetInitiativeTextuelle, onSupprimerTemplateRegles }) {
  ...
  const [dernierTemplateId, setDernierTemplateId] = useState(rulePresetSnapshot?.presetId || '');
  const [nomCourant, setNomCourant] = useState(rulePresetSnapshot?.name || t('rules.preset.custom'));
  ...
  useEffect(() => {
    ...
    setDernierTemplateId('');
    setNomCourant(t('rules.preset.custom'));
  ...
  const relanceInitiativeVisible = false;
  return <div className={`stack rules-panel ${embedded ? '' : 'hub-section panel'}`.trim()}><div><h3>{t('rules.title')}</h3><p className="muted compact-help">{t('rules.help')}</p></div>{embedded && <OptionsProfilCampagne campaignProfile={campaignProfile} onOuvrirProfil={onOuvrirProfilCampagne} />}<OptionsTemplatesReglesCampagne scene={scene} ruleTemplates={ruleTemplates} rulePresetSnapshot={rulePresetSnapshot} dernierTemplateId={dernierTemplateId} nomCourant={nomCourant} editionNom={editionNom} sauvegardeOk={sauvegardeOk} onChangerNom={setNomCourant} onEditerNom={() => setEditionNom(true)} onFermerEditionNom={() => setEditionNom(false)} onAppliquer={appliquerTemplate} onEnregistrer={enregistrerTemplate} onSupprimer={onSupprimerTemplateRegles} /><ResumeReglesActives scene={scene} /><GroupeRegles titre={t('rules.group.development')} aide={t('rules.group.developmentHelp')}><OptionsTemporaliteCampagne scene={scene} temporalite={temporalite} availability={availability} onModifier={onModifierRegles} />{temporalite === temporalityModes.PHASES && <OptionsPhasesCampagne scene={scene} availability={availability} onModifier={onModifierRegles} />}<OptionsSurpriseCampagne scene={scene} availability={availability.surpriseAdvanceOn} onModifier={onModifierRegles} /></GroupeRegles><GroupeRegles titre={t('rules.group.initiative')} aide={t('rules.group.initiativeHelp')}><OptionsUtiliserInitiativeCampagne scene={scene} onModifier={onModifierRegles} />{initiativeActive && <OptionsInitiativeTextuelleCampagne scene={scene} availability={availability.labelInitiative} cardSources={cardSources} onModifier={onModifierRegles} onConfigurer={() => setInitiativeTextuelleOuverte(true)} />}<OptionsOrdreInitiativeCampagne scene={scene} onModifier={onModifierRegles} />{initiativeActive && <OptionsDepartageCampagne scene={scene} rollDefinitions={rollDefinitions} onModifier={onModifierRegles} />}{initiativeActive && temporalite !== temporalityModes.PHASES && <OptionsActionsMultiplesCampagne scene={scene} availability={availability.multipleActionSlots} onModifier={onModifierRegles} />}<OptionsDeclarationCampagne scene={scene} availability={availability.declarationMode} onModifier={onModifierRegles} />{relanceInitiativeVisible && <OptionsRelanceInitiativeCampagne scene={scene} onModifier={onModifierRegles} />}</GroupeRegles><GroupeRegles titre={initiativeActive ? t('rules.group.equalities') : t('rules.group.organization')} aide={initiativeActive ? t('rules.group.equalitiesHelp') : t('rules.group.organizationHelp')}>{initiativeActive && <OptionsEgalitesCampagne scene={scene} onModifier={onModifierRegles} />}<OptionsOrdreCategoriesCampagne scene={scene} onModifier={onModifierRegles} /></GroupeRegles>{initiativeTextuelleOuverte && <FenetreInitiativeTextuelleEdition config={scene?.initiativeTextOrder} cardSources={cardSources} tableSources={tableSources} initiativeTextPresets={initiativeTextPresets} onFermer={() => setInitiativeTextuelleOuverte(false)} onValider={validerInitiativeTextuelle} onEnregistrerPreset={onEnregistrerPresetInitiativeTextuelle} />}</div>;
}
