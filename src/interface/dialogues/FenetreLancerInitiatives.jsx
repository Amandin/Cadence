          ...
          {groupe.participants.map((participant) => {
            ...
            return <div className={`initiative-roll-row multi-roll-row ${surpriseSelectionEnabled ? 'has-surprise' : ''} ${departageManquant ? 'has-tiebreaker' : ''}`} key={participant.id}>
              ...
              {(surpriseSelectionEnabled || departageManquant) && <div className={`initiative-meta-row ${surpriseSelectionEnabled ? 'has-surprise' : ''} ${departageManquant ? 'has-tiebreaker' : ''}`}>
                ...
                {departageManquant && <label className="initiative-tiebreaker-field"><small>{tiebreakerLabel || 'Departage'}</small><input type="number" inputMode="numeric" value={departages[participant.id] ?? ''} onChange={(event) => changerDepartage(participant.id, event.target.value)} /></label>}
              </div>}
...
export function FenetreLancerInitiatives({ participants = [], reserve = [], initiativeTextOrder, initiativeEnabled = true, multipleActionSlots = true, categoryOrder = [], prefillExisting = false, surpriseSelectionEnabled = false, tiebreakerVisible = false, tiebreakerLabel = 'Departage', onFermer, onValider, onPasserHorsInitiative }) {
  ...
  return <Fenetre title={initiativeEnabled ? 'Saisir les initiatives' : 'Preparer la surprise'} onClose={onFermer}>
