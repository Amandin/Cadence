...
function SectionSaisieInitiative({ titre, participants, inclureEnvironnements, valeurs, changerValeur, ajouterAction, retirerAction, textConfig, multipleActionSlots }) {
...
export function FenetreLancerInitiatives({ participants = [], reserve = [], initiativeTextOrder, multipleActionSlots = true, onFermer, onValider, onPasserHorsInitiative }) {
  ...
  const [valeurs, setValeurs] = useState(() => valeursVides(candidats, multipleActionSlots));
  ...
  const vider = () => setValeurs((courant) => ({ ...courant, ...valeursVides(tousEnAttente, multipleActionSlots) }));
  const appliquer = () => { if (restantsAvecValeur.length === 0) return; onValider(valeursRenseignees(restantsAvecValeur, valeurs, multipleActionSlots)); setIdsEnAttente(restantsSansValeur.map((participant) => participant.id)); setValeurs((courant) => ({ ...courant, ...valeursVides(restantsAvecValeur, multipleActionSlots) })); if (restantsSansValeur.length === 0) onFermer(); };
  const ajouterEtConserverAutresInitiatives = () => { if (restantsAvecValeur.length === 0) return; onValider(valeursRenseignees(restantsAvecValeur, valeurs, multipleActionSlots)); onFermer(); };
  ...
  const appliquerEtPasserRestantsHorsInitiative = () => { if (restantsAvecValeur.length > 0) onValider(valeursRenseignees(restantsAvecValeur, valeurs, multipleActionSlots)); passerRestantsHorsInitiative(); };
  ...
  return <Fenetre title="Saisir les initiatives" onClose={onFermer}>
    <div className="initiative-roll-panel">
      ...
      {aucunCandidatAffiche ? <p className="muted">Aucun personnage a renseigner.</p> : <><SectionSaisieInitiative titre="En initiative" participants={participantsEnAttente} inclureEnvironnements={inclureEnvironnements} valeurs={valeurs} changerValeur={changerValeur} ajouterAction={ajouterAction} retirerAction={retirerAction} textConfig={textConfig} multipleActionSlots={multipleActionSlots} /><SectionSaisieInitiative titre="Reserve" participants={reserveEnAttente} inclureEnvironnements={inclureEnvironnements} valeurs={valeurs} changerValeur={changerValeur} ajouterAction={ajouterAction} retirerAction={retirerAction} textConfig={textConfig} multipleActionSlots={multipleActionSlots} /></>}
