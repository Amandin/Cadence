...
export function Suivi({ suivi, onModifier, onSupprimer, avantTitre = null, couleur = 'slate' }) {
  ...
  if (suivi.type === 'clock') {
    return <div className={`tracker ${classeSeuils} ${declenche ? 'triggered' : ''} ${suivi.frozen ? 'frozen' : ''}`} style={styleSeuil}><div className="tracker-top clock-top"><div className="clock-title-zone"><TitreSuivi titre={suivi.name} avantTitre={avantTitre} /><SeuilsActifs seuils={seuils} />{suivi.frozen && <span className="chip">figée</span>}{declenche && <span className="chip hot">À résoudre</span>}</div><div className="clock-inline"><button onClick={() => appliquerPas(-1)}>-</button><HorlogeSuivi suivi={suivi} /><button onClick={() => appliquerPas(1)}>+</button></div><button className={`freeze-btn ${suivi.frozen ? 'active' : ''}`} onClick={() => modifier({ frozen: !suivi.frozen })} title={suivi.frozen ? 'Dégeler cette horloge' : 'Figer cette horloge'} aria-label={suivi.frozen ? 'Dégeler cette horloge' : 'Figer cette horloge'}><IconeMetronome fige={!!suivi.frozen} /></button></div>{declenche && <div className="stack" style={{ marginTop: 8 }}><button className="primary" onClick={() => modifier({ current: 0 })}>Relancer à 0</button><button className="danger-btn" onClick={onSupprimer}>Supprimer</button></div>}</div>;
  }
  ...
  if (suivi.type === 'number') {
    return <div className={`tracker ${classeSeuils} ${declenche ? 'triggered' : ''}`} style={styleSeuil}><div className="tracker-top"><TitreSuivi titre={suivi.name} avantTitre={avantTitre} /><SeuilsActifs seuils={seuils} />{declenche && <span className="chip hot">À résoudre</span>}</div><CompteursSuivi suivi={suivi} onModifier={modifier} />{declenche && <div className="stack" style={{ marginTop: 8 }}><button className="danger-btn" onClick={onSupprimer}>Supprimer</button></div>}</div>;
  }

  if (suivi.type === 'bar') {
    return <div className={`tracker ${classeSeuils} ${declenche ? 'triggered' : ''}`} style={styleSeuil}><div className="tracker-top"><TitreSuivi titre={suivi.name} avantTitre={avantTitre} /><SeuilsActifs seuils={seuils} />{declenche && <span className="chip hot">À résoudre</span>}</div>{deltaOuvert && <div className="delta-pop tracker-action-pop"><label><small>{modeSaisieBarre === 'value' ? 'Nouvelle valeur' : directionSaisieBarre < 0 ? 'Valeur à soustraire' : 'Valeur à ajouter'}</small><input ref={champDeltaRef} type="number" inputMode="numeric" min={modeSaisieBarre === 'delta' ? '0' : undefined} value={delta} onChange={(event) => setDelta(event.target.value)} onKeyDown={(event) => event.key === 'Enter' && appliquerSaisieBarre()} placeholder={modeSaisieBarre === 'value' ? '' : '3'} /></label><button onClick={appliquerSaisieBarre}>OK</button></div>}<div className="controls bar-controls"><button onClick={() => ouvrirDelta(-1)}>-</button><button className="bar-action-zone" onClick={ouvrirValeurBarre} aria-label={`Saisir une nouvelle valeur pour ${suivi.name}`}><BarreSuivi suivi={suivi} /></button><button onClick={() => ouvrirDelta(1)}>+</button></div>{declenche && <div className="stack" style={{ marginTop: 8 }}><button className="primary" onClick={() => modifier({ current: 0 })}>Relancer à 0</button><button className="danger-btn" onClick={onSupprimer}>Supprimer</button></div>}</div>;
  }

  return <div className={`tracker ${classeSeuils} ${declenche ? 'triggered' : ''}`} style={styleSeuil}><div className="tracker-top"><TitreSuivi titre={suivi.name} avantTitre={avantTitre} suffixe={suffixePuces} /><SeuilsActifs seuils={seuils} />{declenche && <span className="chip hot">À résoudre</span>}</div><div className="points-controls"><PointsSuivi suivi={suivi} onModifier={modifier} /></div>{declenche && <div className="stack" style={{ marginTop: 8 }}><button className="primary" onClick={() => modifier({ current: 0 })}>Relancer à 0</button><button className="danger-btn" onClick={onSupprimer}>Supprimer</button></div>}</div>;
}
