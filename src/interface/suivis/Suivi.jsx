...
export function Suivi({ suivi, onModifier, onSupprimer, avantTitre = null }) {
  ...
  if (suivi.type === 'clock') {
    return <div className={`tracker ${declenche ? 'triggered' : ''} ${suivi.frozen ? 'frozen' : ''}`}><div className="tracker-top clock-top"><div className="clock-title-zone"><TitreSuivi titre={suivi.name} avantTitre={avantTitre} />{suivi.frozen && <span className="chip">figé</span>}{declenche && <span className="chip hot">À résoudre</span>}</div><div className="clock-inline"><button onClick={() => appliquerPas(-1)}>−</button><HorlogeSuivi suivi={suivi} /><button onClick={() => appliquerPas(1)}>+</button></div><button className={`freeze-btn ${suivi.frozen ? 'active' : ''}`} onClick={() => modifier({ frozen: !suivi.frozen })} title={suivi.frozen ? 'Dégeler cette horloge' : 'Figer cette horloge'} aria-label={suivi.frozen ? 'Dégeler cette horloge' : 'Figer cette horloge'}><IconeMetronome fige={!!suivi.frozen} /></button></div>{declenche && <div className="stack" style={{ marginTop: 8 }}><button className="primary" onClick={() => modifier({ current: 0 })}>Relancer à 0</button><button className="danger-btn" onClick={onSupprimer}>Supprimer</button></div>}</div>;
  }
