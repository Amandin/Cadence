    ...
    <Fenetre title="Suivi global" onClose={onFermer}>
      <div className="stack">
        ...
        {estHorloge && <div className="grid2">
          ...
          <label className="field">Declencheur<select value={courant.trigger || (courant.auto ? 'round' : 'manual')} onChange={(event) => modifier({ trigger: event.target.value, auto: event.target.value === 'round' || event.target.value === 'phase' })}><option value="manual">Manuel</option><option value="round">Nouveau round</option><option value="phase">Nouvelle phase</option></select></label>
        </div>}
