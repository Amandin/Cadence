...
function GlobalTracker({ tracker, onStep, onOpen, tick }) {
  ...
  return <div className={`global-mini ${tracker.auto ? 'auto-active' : ''} ${tick ? 'auto-tick' : ''} ${maxed ? 'maxed' : ''}`}><button onClick={(e) => { e.stopPropagation(); onStep(-1); }}>−</button><div className="global-mini-main"><span>{tracker.name || 'Menace'}</span><button className={`clock-face global-clock ${tracker.mode === 'counter' ? 'counter-mode' : ''} ${maxed ? 'triggered' : ''}`} style={{ '--clock-progress': `${progress * 360}deg` }} onClick={onOpen} aria-label="Gérer le compteur de scène"><span>{current}</span>{tracker.mode === 'clock' && <small>/{max}</small>}</button>{tick && <em className="auto-plus">+1</em>}</div><button onClick={(e) => { e.stopPropagation(); onStep(1); }}>+</button></div>;
}
