...
function GlobalTracker({ tracker, onStep }) {
  ...
  return <div className={`global-tracker ${tracker.mode === 'clock' ? 'clock-mode' : ''}`}><button onClick={() => onStep(-1)}>−</button><div><span>{tracker.name || 'Menace'}</span><strong>{tracker.current}{tracker.mode === 'clock' ? `/${tracker.max}` : ''}</strong>{tracker.auto && <small>auto/round</small>}</div><button onClick={() => onStep(1)}>+</button></div>;
}

function ReserveCard({ participant, onJoin, onOpen }) {
  const [collapsed, setCollapsed] = useState(false);
  return <div className={`card reserve-card ${collapsed ? 'collapsed' : ''}`} data-participant-id={participant.id}><div className="card-head"><button className="icon-btn collapse-btn" onClick={() => setCollapsed((value) => !value)} aria-label={collapsed ? 'Dérouler la fichette' : 'Enrouler la fichette'}>{collapsed ? '⌄' : '⌃'}</button><button className="card-main" onClick={onOpen}><Avatar participant={participant} /><div className="info"><strong>{participant.name}</strong><div className="muted">{participant.description}</div></div></button><button className="small-btn" onClick={onJoin}>Rejoindre init</button></div>{!collapsed && <div className="trackers"><div className="name-line">{participant.statuses?.map((s) => <span className={`status ${s.duration == null ? 'permanent' : s.loop ? 'loop' : 'temporary'} ${s.expired ? 'expired' : ''}`} key={s.id}>{s.name}</span>)}</div>{participant.trackers?.length > 0 && <div className="muted">{participant.trackers.length} suivi(s) hors initiative · clique la fiche pour modifier</div>}</div>}</div>;
}
