...
function GlobalTracker({ tracker, onStep }) {
  ...
  return <div className="global-mini"><button onClick={() => onStep(-1)}>−</button><div className="global-mini-main"><span>{tracker.name || 'Menace'}</span><button className={`clock-face global-clock ${tracker.mode === 'clock' && current >= max ? 'triggered' : ''}`} style={{ '--clock-progress': `${progress * 360}deg` }} onClick={() => onStep(1)}><span>{current}</span>{tracker.mode === 'clock' && <small>/{max}</small>}</button></div><button onClick={() => onStep(1)}>+</button></div>;
}
