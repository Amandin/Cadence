...
function GlobalTracker({ tracker, onStep, onOpen, tick }) {
  ...
  const progress = tracker.mode === 'clock' ? Math.min(1, current / max) : 0;
  return <div className={`global-mini ${tracker.auto ? 'auto-active' : ''} ${tick ? 'auto-tick' : ''}`}><button onClick={(e) => { e.stopPropagation(); onStep(-1); }}>−</button><div className="global-mini-main"><span>{tracker.name || 'Menace'}</span><button className={`clock-face global-clock ${tracker.mode === 'clock' && current >= max ? 'triggered' : ''}`} style={{ '--clock-progress': `${progress * 360}deg` }} onClick={onOpen} aria-label="Gérer le compteur de scène"><span>{current}</span>{tracker.mode === 'clock' && <small>/{max}</small>}</button>{tick && <em className="auto-plus">+1</em>}</div><button onClick={(e) => { e.stopPropagation(); onStep(1); }}>+</button></div>;
}
...
function GlobalTrackerSheet({ tracker, onChange, onStep, onClose }) {
  ...
  return <Sheet title="Compteur de scène" onClose={onClose}><div className="stack"><label className="row"><input type="checkbox" checked={!!current.enabled} onChange={(e) => patch({ enabled: e.target.checked })} /> actif dans l’entête</label><label className="field">Nom<input value={current.name || ''} onChange={(e) => patch({ name: e.target.value })} placeholder="Menace" /></label><div className="grid2"><label className="field">Type<select value={current.mode || 'clock'} onChange={(e) => patch({ mode: e.target.value })}><option value="clock">Horloge</option><option value="counter">Compteur</option></select></label><label className="field">Valeur<input type="number" inputMode="numeric" value={current.current ?? 0} onChange={(e) => patch({ current: e.target.value === '' ? 0 : Number(e.target.value) })} /></label></div>{current.mode === 'clock' && <label className="field">Maximum<input type="number" inputMode="numeric" min="1" value={current.max ?? 10} onChange={(e) => patch({ max: Math.max(1, Number(e.target.value) || 1) })} /></label>}<label className="row"><input type="checkbox" checked={!!current.auto} onChange={(e) => patch({ auto: e.target.checked })} /> avance automatiquement à chaque nouveau round</label><div className="grid2"><button className="small-btn" onClick={() => onStep(-1)}>−1</button><button className="small-btn" onClick={() => onStep(1)}>+1</button></div></div></Sheet>;
}
