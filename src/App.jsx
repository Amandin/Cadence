...
export default function App() {
  ...
  const retourPreparationVisible = scene.round >= 0 && (temporaliteSouple ? (scene.historiqueSouple || []).length === 0 && scene.round === roundDepartScene : temporalitePhases ? scene.round === roundDepartScene && (scene.phase || 1) <= 1 && (phaseCurrentIndex <= 0 || !phaseParticipants.length) : scene.round === roundDepartScene && currentIndex <= 0);
  const retourPossible = scene.round < 0 ? false : temporaliteSouple ? (scene.historiqueSouple || []).length > 0 || scene.round === scene.startRound : temporalitePhases ? phaseCurrentIndex > 0 || (scene.round === scene.startRound && (scene.phase || 1) <= 1 && (phaseCurrentIndex <= 0 || !phaseParticipants.length)) : scene.participants.length > 0 && (scene.round > 0 || currentIndex > 0);
  ...
  useEffect(() => {
    ...
    if (currentView === 'scene' && scene.round > roundAvant && scene.temporalite === temporalityModes.PHASES && scene.phaseRerollEachRound) setInitiativeEntryOpen(true);
  ...
  const nextTurn = (direction) => {
    ...
    if (direction > 0 && scene.promptInitiativeOnNext && !temporaliteSouple && scene.round >= 0 && participantAjustementInitiative && !skipInitiativeAdjustPromptRef.current) {
  ...
  const openInitiativeEntry = () => { setOpenMenu(false); setInitiativeEntryOpen(true); };
  const createBlankCharacter = (options) => { const initiative = Number(options?.initiative); characters.addCharacter(Number.isFinite(initiative) ? { ...createBlankParticipant(), initiative, actionSlots: [{ id: 'slot-1', initiative, order: 0 }] } : createBlankParticipant(), options); setAddSheetOpen(false); };
  ...
  const chooseScene = (index) => { actions.setSceneIndex(index); setCurrentView('scene'); };
  ...
  const nextLabel = scene.round < 0 ? 'Commencer' : temporaliteSouple ? toutLeMondeAJoueSouple ? 'Nouveau round' : 'Mode souple : choisir dans la liste' : blocked.length ? 'Gérer horloge bloquante' : temporalitePhases ? phaseAttendRelanceInitiative ? 'Saisir les initiatives' : !phaseParticipants.length ? 'Aucun participant actif' : phaseEnFin && phaseSuivanteDisponible ? 'Phase suivante' : phaseDemarreNouveauRound ? 'Nouveau round' : 'Participant suivant' : nextStartsRound ? 'Nouveau round' : 'Participant suivant';
  const classeSuivantEffective = scene.round < 0 ? 'next-round' : temporaliteSouple && toutLeMondeAJoueSouple ? 'next-round' : blocked.length ? 'blocked' : temporalitePhases ? phaseDemarreNouveauRound ? 'next-round' : phaseEnFin && phaseSuivanteDisponible ? 'next-phase' : '' : nextClass;
  ...
  const libelleBas = scene.round < 0 ? 'Commencer' : temporaliteSouple ? toutLeMondeAJoueSouple ? `Nouveau round · R${scene.round + 1}` : 'Choisir' : temporalitePhases ? phaseAttendRelanceInitiative ? 'Init requise' : phaseDemarreNouveauRound ? `Nouveau round · R${scene.round + 1}` : phaseEnFin && phaseSuivanteDisponible ? `Phase ${scene.phase + 1}` : `Suivant · P${scene.phase}` : nextStartsRound ? `Nouveau round · R${scene.round + 1}` : `Suivant · ${libelleRoundCourant}`;

  const fenetresCommunes = <>
    <FenetresSuperposees campaignName={campaignName} scene={scene} restorePoints={restorePoints} dark={dark} characters={characters} templates={templates} actions={actions} etatInterface={{ addSheetOpen, openMenu: currentView === 'scene' && openMenu, notice, globalSheetOpen, clockModalOpen, initiativeEntryOpen: currentView === 'scene' && initiativeEntryOpen }} commandesInterface={{ ouvrirAjoutPersonnage: openAddCharacter, ouvrirSaisieInitiatives: openInitiativeEntry, ouvrirHubCampagne: () => setCurrentView('hub'), fermerAjoutPersonnage: () => setAddSheetOpen(false), fermerMenu: () => setOpenMenu(false), fermerNotice: () => setNotice(null), fermerCompteurGlobal: () => setGlobalSheetOpen(false), fermerResolutionHorloge: () => setClockModalOpen(false), fermerSaisieInitiatives: () => setInitiativeEntryOpen(false), ouvrirSauvegardeTemplate: openTemplateSave, creerPersonnageVierge: createBlankCharacter, creerDepuisTemplate: createFromTemplate, restaurerScene: restoreScene, retourPreparation: returnToPreparation, avancerRound: advanceRound, resetSuivisScene: resetSceneTrackers, effacerEtatsScene: clearSceneStatuses }} compteurGlobal={{ horlogesBloquantes: blocked }} resolutionHorloge={{ resetClock, deleteClock }} templatesUi={{ templateTarget, templateError, fermerSauvegardeTemplate: () => setTemplateTarget(null), enregistrerTemplate: saveTemplate }} />
