...
function GlobalTracker({ tracker, onStep }) {
  ...
  const pseudoTracker = { id: 'global-scene-tracker', type: 'clock', name: tracker.name || 'Menace', current: tracker.current || 0, max: tracker.mode === 'clock' ? tracker.max || 10 : Math.max(10, tracker.current || 0), auto: tracker.auto };
  return <div className="global-tracker-inline"><button onClick={() => onStep(-1)}>−</button><Tracker tracker={pseudoTracker} onChange={(next) => onStep((next.current || 0) - (tracker.current || 0))} onDelete={() => {}} /><button onClick={() => onStep(1)}>+</button></div>;
}
